...
LAYOUT_GEOMETRY = {'column_bytes': 32, 'row_bytes': 1024, 'bank_bytes': 16777216, 'token_lanes': 16, 'banks_per_group': 4, 'channels': 16, 'channel_bytes': 1073741824}
LAYOUT_REVISION = 'stable-row-arena-v1'

def object_layout(objects,dhead,dbyte):
    """Stable within a case/layer policy if capacity_tokens is the lifetime bound."""
    g=LAYOUT_GEOMETRY;cursor=0;result={}
    vector_lanes=g['banks_per_group']*(g['column_bytes']//dbyte)
    for obj in objects:
        tokens=obj['tokens'];capacity=obj.get('capacity_tokens',tokens)
        if capacity<tokens:raise ValueError('Object exceeded its lifetime allocation: '+obj['id'])
        columns=math.ceil(capacity/g['token_lanes'])*math.ceil(dhead/vector_lanes)
        span=math.ceil(columns*g['column_bytes']/g['row_bytes'])*g['row_bytes']
        result[obj['id']]=dict(K_base=cursor,V_base=(1<<23)+cursor,span_bytes=span,capacity_tokens=capacity,
            reservation_source='capacity_tokens' if 'capacity_tokens' in obj else 'current_tokens_only')
        cursor+=span
    if cursor>(1<<23) or (1<<23)+cursor>g['bank_bytes']:
        raise ValueError('Shared object arena exceeds native K/V bank ranges; no bank/channel remapping is allowed.')
    return result
...
class TraceBank:
    ...
    def base(self,n,h,dhead=128,dbyte=2):
        ...
        assert n>0 and h>0
        folder=self.root/f'native-D{dhead}-B{dbyte}-N{n}-H{h}';folder.mkdir(exist_ok=True)
        if (folder/'input.trace').exists() and (folder/'generator.json').exists():
            prior=json.loads((folder/'generator.json').read_text())
            if prior.get('returncode')==0:
                lines=(folder/'input.trace').read_text().splitlines();self.bases[key]=lines;return lines
        path=folder/'input.trace'
        cmd=[sys.executable,str(REPO/'pim_ramulator_src/trace_gen/gen_trace_attacc_bank.py'),
             '-dh',str(dhead),'-nh',str(h),'-l',str(n),'-maxl',str(max(4096,n)),'-db',str(dbyte),'-o',str(path)]
    ...
    def run(self,name,lines,residents,metadata=None):
        ...
        if metadata is not None:
            private=[metadata['physical_layout'][o['id']] for o in metadata['objects'] if o.get('private',False)]
            clocks=[]
            for f in folder.glob('commands.ch*'):
                for line in f.open():
                    ...
                    if any(o['K_base']<=local<o['K_base']+o['span_bytes'] for o in private):clocks.append(int(fields[0]))
    ...
    def prepare(self,keys,dhead=128,dbyte=2):
        ...
        for n,h,r,d,b in keys:self.base(n,h,d,b)
        def one(key):
            n,h,r,d,b=key;lines=expand(self.bases[n,h,d,b],r)
            row=self.run(f'scan-D{d}-B{b}-N{n}-H{h}-Q{r}',lines,[r]*sum(s.startswith('PIM_MAC_AB') for s in lines))
            row.update(dhead=d,dbyte=b,layout_revision=LAYOUT_REVISION)
            return key,row
        with ThreadPoolExecutor(max_workers=JOBS) as pool:
            for key,row in pool.map(one,keys):self.profiles[key]=row

    def get(self,n,h,r=1,dhead=128,dbyte=2):
        key=(n,h,r,dhead,dbyte)
        if key not in self.profiles:self.prepare([(n,h,r)],dhead,dbyte)
    ...
    def shared(self,name,objects,views,h,mq,dhead=128,dbyte=2):
        ...
        """
        assert h<=LAYOUT_GEOMETRY['channels']
        allocation=object_layout(objects,dhead,dbyte)
        ...
        phases={};sfm=None
        for obj in objects:
            src=self.base(obj['tokens'],h,dhead,dbyte)
        ...
        def emit(oid,queries,phase):
            offset=allocation[oid]['K_base'];start=len(lines);r=len(queries)
        ...
        metadata=dict(objects=objects,views=views,query_ranges=query_ranges,blocks=blocks,
            physical_layout=allocation,dhead=dhead,dbyte=dbyte,layout_revision=LAYOUT_REVISION,
