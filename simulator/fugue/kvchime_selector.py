...
class Selector:
    def __init__(self,profiles,h,group_size=1,dhead=128,dbyte=2):
        self.group_size=group_size
        # These two shapes are exclusively calibration inputs for every head count.
        p1=profiles.get(1024,h,8,dhead=dhead,dbyte=dbyte);p2=profiles.get(4096,h,8,dhead=dhead,dbyte=dbyte)
        self.a=(p2['scan_us']-p1['scan_us'])/(4096-1024)
        self.b=p1['scan_us']-1024*self.a
        self.calibration=dict(dhead=dhead,dbyte=dbyte,layout_revision='stable-row-arena-v1',heads_per_HBM=h,n1=1024,n2=4096,q=8,
