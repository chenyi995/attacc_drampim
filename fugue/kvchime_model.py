...
def geometry(name):
    if name=='LLAMA3.1-8B':
        return dict(name=name,ndec=32,hdim=4096,num_heads=32,num_kv_heads=8,dhead=128,ff_scale=3.5,gqa_size=4,dtype=DataType.W16A16)
...
class Model:
    def __init__(self,name,tp,bank):
        ...
        assert self.m['num_heads']%tp==0 and self.m['dhead']==128
        self.gqa=self.m['gqa_size'];self.kvheads=self.heads//self.gqa
        assert self.m['num_kv_heads']%tp==0
        self.h=math.ceil(self.kvheads/5);self.qbytes=self.heads*128*2;self.kvbytes=2*self.kvheads*128*2
        ...
        self.bank=bank
        self.bank.prepare([(1024,self.h,8),(4096,self.h,8)])
        self.selector=Selector(self.bank,self.h,group_size=self.gqa);self.ops={};self.shared_cache={}
    ...
    def common(self,qkv,q,n,b,decode=False):
        ...
        for l in ls:
            ...
            if l.name=='qkv':
                l.m=b*qkv;l.n=(self.heads+2*self.kvheads)*128
    ...
    def gpu_attention(self,q,n,b):
        ...
        v=[self.op('sum','score',LayerType.MATMUL,q*self.gqa,n,128,self.kvheads*b),
           self.op('sum','softmax',LayerType.SOFTMAX,q,n,1,self.heads*b),
           self.op('sum','context',LayerType.MATMUL,q*self.gqa,128,n,self.kvheads*b)]
    ...
    def dense_scan(self,q,n,b,mq=False):
        ...
        self.bank.prepare([(n,h,r) for r in set(rs)])
        return self.combine([self.bank.get(n,h,r) for r in rs])
    @staticmethod
    def combine(rows):
    ...
    def shared_scan(self,objects,views,mq):
        ...
        def one(tile):
            key=hashlib.sha256(json.dumps([self.h,objects,tile,mq],sort_keys=True,separators=(',',':')).encode()).hexdigest()[:20]
            if key not in self.bank.shared_cache:
                self.bank.shared_cache[key]=self.bank.shared('shared-'+key,objects,tile,self.h,mq)
        ...
        for v in vs:
            ...
            for start in range(0,count,8):
                tile=deepcopy(v);tile['query_count']=min(8,count-start)
    ...
    def scan_energy(self,row,q,n,b):
        ...
        cal=q*n*128*self.heads*b*ENERGY_TABLE['PIM'][PIMType.BA]['alu']
