"""Verify current results and independently redraw every single-metric package."""
...
def verify():
    ...
    def one(record):
        ...
        with tempfile.TemporaryDirectory(prefix='KVChime-independent-figure-') as tmp:
            ...
            assert load(t/'render-check.json')['axes']==1
        return dict(path=record['path'],png_sha256=sha(d/'figure.png'),raw_rows_match_simulation=True,independent_PNG_identical=True,single_response_axis=True)
