# decode_scan_ms_per_token

本图只表示 **Decode scan speedup over F2**。模型是外层分组，workload 是内层分组；性能图表示 baseline latency / 本方案 latency（越大越快），容量图表示本方案 / F0（越小占用越少）。所有绝对值保存在原始数据中。
