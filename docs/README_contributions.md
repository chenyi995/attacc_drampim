...
**怎么做：** 保持 H 的 master 分布不变，把 A 在 H 下的 D0–D7 紧凑追加进 H 的 diff 行；64 token 只占一行，这一行落在 diff 行轮转到的那个 channel（例中记为 ch3）；修正超过一行时，下一行轮到下一个 channel。其他 KV head 的 diff 各自处理。
