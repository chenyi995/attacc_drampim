...
(块 08 §6)供用户抽查。审计由助手撰写,自我审计的局限与缓解见
块 00 §5。
...
| 04 | `04_mq_c_series.md` | C 系列:MQ-MAC 批命令(trace `--mq` + 时序模型 + DAG 集成)、非对称相位、D_i 位图、bank-whole prefill、总线转向 C++ | 用户+助手,`9d6fc7b`/`264d14a`/`3e338e6`/`711ae25`(2026-08-21) |
| 05 | `05_agentic_history.md` | agentic 多轮驻留 KV(`history_len`):解析式半 + 事件 DAG 半 | 用户+助手,未提交(2026-08-22) |
| 06 | `06_area_balance_0822.md` | 专题:MQ bank-PE 面积平衡点(容量×速率两轴、in-bank 预算、换算链)全链条审计 | 用户+助手,2026-08-22 会话 |
...
暂无论文实验编号。数字进论文正文前须用户手动复现(论文仓库 CLAUDE.md
的数据纪律)。
...
xinyao;`9d6fc7b` 起(分支 chenyi-experiment-821)为用户+助手会话;
