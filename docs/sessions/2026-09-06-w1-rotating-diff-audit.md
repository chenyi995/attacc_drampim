...
chenyi9 询问这份 workload 为什么会产生 row conflict。新增只读说明脚本 `audit/2026-09-06/explain_w1_row_conflict.py`，从真实W1账本提取末轮main共读的worker回答、自身历史及各档地址；输出 [W1_ROW_CONFLICT.md](../../audit/2026-09-06/W1_ROW_CONFLICT.md) 和同名JSON。

核实周期性master预约使同一worker的回答反复落在两个channel，main自身历史还与回答较多的channel重叠。保留A3b/A4c与A4e的具体共读地址对，解释同banks不同行的切换与可用通道并行度。额外记录一个重要范围：A4e并没有把worker回答这个子集单独均分到所有通道；它改善的是包含main自身历史的完整读集，不能用子集统计冒充整体收益。

没有修改实现、输入或论文；没有运行Ramulator、测量ACT或时延。解释中的数字由保存的脚本从地址计算，没有给A3b添加人为行冲突罚时。
