...
| R16 | **decode batch=1 埋没全部布局差异**(权重流按并发数虚增,PIM 扫描全藏其下;AttAcc 原版本就重叠) | **全档 decode 服务批宽 8**(GPU 权重一遍服务全波,KV 每查询各自拉;A2 重写为同构波结构);**MQ n_cap 顶格 8**(512 B/1.733 GHz 配平,修订 R7 的 12 档;**2026-08-27 再修 1.3004 GHz/8 tCK → R19**;MQ 仅 A5/A6,其余档连 prefill 也不开);wl_tiny 实测 A2 1.81→1.51 s、A4 1.29→0.98 s | `c163936` |
