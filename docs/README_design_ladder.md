...
## 8. 实测证据（`CACHEBLEND-TINY`，`--num-hbm 2 --ngpu 1`，`heads_per_hbm=4`，k=8）

> ⚠️ **本节表格是 serial 模式（无 `--pipeopt`）跑的。** 裁决（chenyi9 2026-09-04 深夜）：`--pipeopt`
> **必须常开**（现已默认 ON），因为 serial 模式下空通道不能接独立的活，抹掉布局省下的总工作量。
> pipeopt 全量重跑进行中；已有的 `mr_R2` 对照显示 pipeopt 只改 0.7–1.0%、排名不变
> （A4c 1.0066×、A4d 1.0046× vs A3b），见 `sessions/2026-09-04.md` §21。
> **另外 A4c / A4d 的 master 落点已改为持久的写入序轮转（§3.1），本节它们的数也已作废**；
> A4c / A4d / A4e 三档在 pipeopt 下重跑中。跑完本节整体替换。

三个人造的多轮 workload（`workload/handcheck/gen_multiround.py`，消费者上下文
`sys | shared | own | shared | own | …`，每轮取一个块修一次、写一块自己的 KV）。
**全部过 Ramulator，`--engine dag`。** 括号 = A3b / 该档，>1 为更快。

### 8.1 端到端 makespan

| workload | A3b | A4 | A4b | **A4c** | **A4d** |
|---|---:|---:|---:|---:|---:|
| 2 轮 × 每轮修 8 块，3 agent | 0.083033 | 0.9749× | 0.9867× | **1.0081×** | 1.0031× |
| 16 轮 × 每轮修 1 块，3 agent | 0.191380 | 0.9881× | 0.9975× | **1.0047×** | 1.0047× |
| 16 轮 × 每轮修 1 块，8 agent | 0.507571 | — | 0.9992× | **1.0058×** | 1.0058× |

### 8.2 第 0 层 PIM 扫描时间（布局真正作用的那一层）

| workload | A3b | A4 | A4b | **A4c** | **A4d** |
|---|---:|---:|---:|---:|---:|
| 2 轮，3 agent | 0.001472 | 0.7103× | 0.8122× | **1.0788×** | 1.0034× |
| 16 轮，3 agent | 0.002313 | 0.7767× | 0.9203× | **1.0672×** | **1.0672×** |

### 8.3 为什么 A4b 输、A4c/A4d 赢 —— 按扫描类型拆（16 轮，3 agent，秒）
...
| **A4d** | **0.001063（=A3b）** | **0.001012** | **0.002167** |

A4b 在私有段赢了（gather 兑现），却在公共段输更多——那些扫描一个修正都不读，
ch15 全空，它白白少一条 master 通道。A4c/A4d 公共段与 A3b 逐字节相同，私有段拿到同样的收益。

k=8 时 A4c 与 A4d 在 16 轮 workload 上逐位相同：4 个 head 的修正各 120 token，A4c 各占一行，
A4d 合成 480 token = 2 行；两边最忙通道都是 master 主导的 5 次 ACT，max 没动。差别要到 k 大时才在
max 里显出来（§4 的表）。

### 8.4 端到端幅度为什么只有 0.5–0.8%

GPU 占 makespan 91–94%，其中 **prefill attention（$L^2$，A1–A4d 都在 GPU）占 GPU 时间 88.7%**
（8 agent 实测）。布局只能动 PIM 那 6–9%。另外 decode 每步每层有 22.03 us 的固定 kernel 开销
...
GPU 占比没动。**布局消融应看 §8.2 的扫描层面数**；makespan 被 GPU prefill 主导是配置的性质，
不是布局的性质。

### 8.5 证据等级

- **实测**：§8.1–8.3 全部（Ramulator、`--engine dag`）；§3.2、§4 的 ACT 表由放置规则算出，
