...
怎么用、验证到什么程度、还悬置什么。遵守 `docs/OUTPUT_SPEC.md`;
所有数字为实测,来源标注。实现 commit:`882887b`(2026-08-22)。
...
算子维度(GEMM 记号见 `README_fugue_dataflow.md`):score 的列数 N 与
context 的收缩维 K 各加宽 H;M(查询条数)不变。
...
- **暂无论文实验编号**(严格论文模式下 A/C 之外开新系列须宸逸裁决);
  属能力储备,现有 workload JSON 全为 H=0,行为逐位不变;
- 悬置:历史块受 TLB 单块 8 MiB 上限约束(LLAMA-7B 口径单请求
  H ≤ 32768 行),未做多块分裂;
- 完整审计(数据源/等价论证/错位记录)见 `docs/audit/05_agentic_history.md`。
