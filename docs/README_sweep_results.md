...
> # ⚠️ 本页的 makespan 结论已作废（2026-09-02）
>
> 这批数据跑在 **`75da860` 之前的引擎**上。那个提交修掉了一个调度缺陷：没有
> `--pipeopt` 时，调度器把每个事件挂在同一个串行资源上，于是一次 KV 扫描的十六条
> 通道 lane 被**求和**而不是**重叠** —— 硬件上它们并发，扫描应当取最忙的那条通道。
>
> 实测(LLAMA-7B 同一格)：baseline A3 **141.6 s → 19.5 s**，A6 **100.1 s → 15.3 s**。
> **偏差 3.2–7.6 倍且不均匀。**
>
> | 本页内容 | 状态 |
> |---|---|
> | §2 阶梯表的 **makespan 列**、§2 "A6 相对每一档"的 **makespan 列**、按模型表的 makespan | **作废** |
> | §3.3 A6 选边不变量（基于 makespan 比较） | **作废，需在新引擎上重验** |
> | §2 的 **能量**与 **KV link** 列、§3.1（A2 能量低 19%）、§3.2（五档结果雷同） | **仍然有效** —— 能量、链路字节、事件数在修复前后逐格不变 |
> | §5 引擎缺陷（`LLAMA-7B×pipeline` 崩溃） | **仍然有效**，与调度无关 |
>
> 还有一条与调度无关的独立问题：**`--num-hbm 1` 上的 A3b 根本不是 A3b**。切片把
> 一个 head 摊到 `max(1, 16 // heads_per_hbm)` 个通道，LLAMA-7B 的 32 个 KV head
> 挤一个堆栈时 stripe 钳到 1，载荷向量与 A3 逐位相同（见 `ce0cde6`）。所以
> **§3.2 里"A3 与 A3b 分不开"对 LLAMA-7B 而言不是发现，是同一个 run**。
>
> 数据本身原样归档在
> `output/analysis/sweep_20260830_prefix_scheduler/`，一个数都没改 —— 它是
> 对账"修复改变了什么"的基线。
