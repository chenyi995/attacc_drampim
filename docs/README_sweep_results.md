...
2. **8 个任务仍缺 37 档**，补跑进行中；`sweep_completeness.csv` 的
   `rungs_missing` 列逐任务列明。缺档全部由节点磁盘/内存耗尽造成，
   **没有一档是引擎缺陷** —— 唯一例外是 `LLAMA-7B/pipeline` 的 A3/A3a/A3b，
   它们在健康节点上无 traceback 静默崩溃，**未结案**，补跑时会重试。
