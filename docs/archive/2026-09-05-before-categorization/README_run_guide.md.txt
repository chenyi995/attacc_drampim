...
### 5b. B1 矩阵：让 A4c / A4e / A5 / A6 各自分开（2026-09-05 晚，只改 workload）

第一次小跑后的事实：一步 decode 里 GPU 固定工作占大头，布局只能动扫描那一块；A5 只在"长驻留上下文 + 每轮新 token 很少"的请求上赢；
A6 要分流就得同时有话少和话多的 agent；A4e 的表只在共读 chunk 在写入序轮转下撞同一 slot 时有用。B1 据此构造（`turns` 写法）：

| 项 | 值 | 给哪一档杠杆 |
|---|---|---|
| 检索 | 每轮从 64 chunk 语料里**散着**取 2 个（seeded） | A4e：散取的共读 chunk 有一半相隔偶数次写入，stripe 2 下撞同一 slot |
| 每轮 chunk 数 | 2（16 个修正 token = 64 B，远小于一行） | A4c：A3b 每轮修正独占一行，A4c 打包 |
| agent | 8 个：4 个话少（每轮写 16 token）+ 4 个话多（256 token） | A6：话少的轮估价选 PIM，话多的选 GPU；A5 全进 PIM 还要付 16k 语料 ingest |
| 轮数 / lout | 8 / 128 | |

结构探针（`output/analysis/b1_levers.py`，用真实计划、账本和读集，不跑 Ramulator；A5/A6 一列是按 9-05 小跑校准的估价，属推导）：

| 点 | A4e：最忙 lane 行数 表 vs 轮转 | A4c：修正占用的 DRAM 行 A4c vs A3b | A5/A6：估价选 PIM / GPU 的轮数 |
|---|---|---|---|
| B1 | 少 9.5% | 少 44% | 28 / 37 |
| T6 连续检索 | 少 0.1% | 少 44% | 28 / 37 |
| T5 话少占比 0 | 少 6.1% | 少 44% | 0 / 65 |
| T5 话少占比 1 | 少 7.2% | 少 44% | 56 / 9 |
| T1 16 agent | 少 9.5% | **少 0%** | 56 / 73 |
| T2 16 轮 | 少 3.3% | 少 47% | 60 / 69 |

T1 那行的 0% 是当前模拟器的边界：A4c 的 diff 区是每个 head 通道一条全局游标，agent 多了以后各 agent 的修正互相穿插，
一轮一行的收益被抵消（审计 C7 已记为边界；改不改是仿真侧的事，本矩阵不动仿真）。

轴：T1 agent 数 4/**8**/16，T2 轮数 4/**8**/16，T3 每轮 chunk 1/**2**/4，T4 话少 agent 每轮 token 8/**16**/32/64，
T5 话少占比 0/0.25/**0.5**/0.75/1，T6 检索 **散取**/连续，T7 语料 16/32/**64** chunk，T8 lout 32/**128**/512。文件 `workload/probe/sweep/B1_turns.json`、`T*_turns.json`。
