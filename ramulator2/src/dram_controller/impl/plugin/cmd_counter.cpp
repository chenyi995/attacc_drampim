...
class CommandCounter : public IControllerPlugin, public Implementation {
  ...
  public:
    ...
    void finalize() override {
      // Each HBM channel owns a controller instance.  A shared configured
      // path previously let the last controller overwrite every other
      // channel's command counts, which makes chip-wide ACT experiments
      // incorrect.  Keep one deterministic file per channel instead.
      auto channel_path = m_save_path;
      channel_path += fmt::format(".channel{}", m_ctrl->m_channel_id);
      std::ofstream output(channel_path);
