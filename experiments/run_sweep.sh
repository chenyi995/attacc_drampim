...
#   * a BASELINE workload (C1_turns.json, C2_turns.json) runs every combo
#     A1 A2 A3b A4c A4e A5 A6;
#   * a SWEEP point (C1_S*_turns.json) runs A3b and A6 only;
...
#     filter-regex  selects manifest rows by file name, e.g. '^C1_turns' or 'C1_S3_'
...
#        EVENTS (none), KVPIM_SCRATCH (Ramulator dir)
set -u
...
export EPIC_K=${EPIC_K:-8} GPU_MODEL=${GPU_MODEL:-flash} EVENTS=${EVENTS:-none}
...
echo "$(date +%T) sweep start: ${#FILES[@]} workloads, model $MODEL (ngpu $NGPU, hbm $NUM_HBM), gpu_model $GPU_MODEL, k $EPIC_K" >> "$OUTROOT/sweep.log"
is_baseline() { [[ $1 =~ ^C[0-9]+_turns\.json$ ]]; }
