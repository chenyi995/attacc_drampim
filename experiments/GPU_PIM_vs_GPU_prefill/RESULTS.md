...
（直接扫 `--epic-prefix-recompute-tokens` p = 1…256 得到，单段 L = 8192、无新 query；每行是 A6 = A4 的插值点。）EPIC 默认 p = 1，远在上限内：协同 prefill 快 2×（NVLink3，A6/A4 ≈ 0.5）到 5–6×（PCIe4，≈ 0.16–0.21）；p 越大协同优势越小，超过上限后纯 GPU 更快。这个 p 上限几乎不随复用段长度 L 变化（直接扫 L 验证）：

| 模型 | 链路 | L=1024 | L=2048 | L=8192 | L=32768 |
|---|---|---:|---:|---:|---:|
| LLAMA-7B | NVLink3 | p ≤ 18 | 21 | 22 | 22 |
| LLAMA-7B | PCIe4 | 93 | 93 | 89 | 87 |
| GPT-175B | NVLink3 | – | 35 | 35 | 35 |
| GPT-175B | PCIe4 | – | 206 | 210 | 209 |

L 只改变拐点两侧的差距：p = 1 时 7B 的 A6/A4 在 NVLink3 下从 0.89（L=1k）降到 0.25（L=32k），PCIe4 下从 0.66 降到 0.09——纯 GPU 读回 K/V 和对它做 skinny attention 的代价 ∝ L，PIM 一趟扫描也 ∝ L 但便宜得多；而 p 每增加 1 两边各增加的代价同样 ∝ L，所以相等点的 p 不动。
