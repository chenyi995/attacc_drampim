...
计算时实现为 `074cbeb`，结束时 HEAD 为 `074cbeb`；中间提交只更新文档/报表标题，本轮涉及的实现与输入哈希相同。设计改动来自 `753044b`，论文为 `8fe2022`。源码、输入和版本指纹在 [provenance](evidence/provenance.json)，结束核对在 [derived](evidence/derived.json)。
...
- 220 组列地址与真实 trace generator 逐条一致。此项证明审计数字忠实于生成器；它不代表所有列请求都严格落在 token 的存储边界内，见第 6 节。
...
```text
main context = 16 + 24×16 + 23×128 + 22×2×128 = 8976 token
main diff    = 22×2×8 = 352 token
其中旧 diff = 336，本轮新 diff = 16
main 本轮重算 = 16 token 指令 + 2×8 修正 = 32
...
| main | 档位 | diff K行总数 | diff QK列请求总数 | full QK列请求总数 | full最忙通道K行 | full最忙通道QK列请求 |
|---|---|---|---|---|---|---|
| g00 | A3b | 22 | 44 | 1166 | 23 | 368 |
| g01 | A3b | 22 | 44 | 1166 | 29 | 380 |
| g00 | A4c | 6 | 88 | 1210 | 24 | 380 |
| g01 | A4c | 6 | 88 | 1210 | 24 | 384 |
| g00 | A4e | 2 | 88 | 1210 | 12 | 192 |
| g01 | A4e | 2 | 88 | 1210 | 13 | 192 |

**A4c：当前实现没有充分兑现连续布局的收益。** diff 从 22 行降到 6 行，减少 72.73%。A3b 把同轮两份 8-token diff 连成 16-token burst；A4c/A4e 虽然也已将它们连续存放，扫描端却按两个对象分别取整，使 main 的 diff 列请求从 44 增到 88，full 从 1166 增到 1210。这是本仓库扫描描述符未合并的实现遗漏，不能解释成 A4c 本来就需要更多列命令。合并同一扫描所需的物理连续段后，原来的额外取整可以消除；跨轮同样成立。下表计数仍保留为修复前的真实代码行为，不能据此否定布局设计。完整scan/TBT需在修复后重新计价。

**A4e：当前最明确的是通道均衡。** g00 main 的最忙通道从 380 个 QK 列请求降到 192，下降 49.47%；g01 也约减半。独立核查加入本轮已生成的 127 个 token 后，这两个最忙通道计数保持不变，因此结论不只是 decode 第一步的现象。

不能把两个 main 代替整个 batch。最后一轮六个请求分别如下，worker 1 原本较均衡，A4e 在它身上反而增加了最忙通道负载：

| 请求 | A3b最忙QK列请求 | A4c最忙QK列请求 | A4e最忙QK列请求 |
|---|---|---|---|
| g00_m_t023 | 368 | 380 | 192 |
| g00_w0_t023 | 396 | 384 | 244 |
| g00_w1_t023 | 228 | 224 | 244 |
| g01_m_t023 | 380 | 384 | 192 |
| g01_w0_t023 | 384 | 390 | 244 |
| g01_w1_t023 | 216 | 226 | 244 |

各请求的通道服务还会共享资源。为了便于检查，这里也保存同一层六个查询的 QK 工作量逐通道相加后的结果；它只反映共享通道工作量，没有 QK/PV、softmax、链路和 GPU 的完整时序：

| 档 | 六请求QK列请求按ch0…ch7相加 | max |
|---|---|---|
| A3b | 490,1316,490,1318,496,1332,488,1338 | 1338 |
| A4c | 520,1348,520,1348,520,1346,456,1298 | 1348 |
| A4e | 1004,1140,712,996,834,700,1150,820 | 1150 |

整个batch最忙通道的累计QK工作量，A4c→A4e只降低 14.69%。这进一步说明：两个汇总者各自约减半的列负载，不能直接代表整批scan，更不能代表TBT。
### 仅对布局计数成立的上限

- 352 个 diff token 最少占 `ceil(352/256)=2` 个 K 行。相对 A3b 的 22 行，最多减少 90.91%；A4c 的 6 行拿到了可省行数的 80.00%，A4e 的 2 行达到这个**占行数下界**。
- 固定 A4c/A4e 的 1210 个 QK 列请求，八通道任意均分的乐观下界为 `ceil(1210/8)=152` 个/通道。A4e 的 192 个相当于拿到从 380 到 152 这段均衡空间的 82.46%。这只是忽略不可拆对象/其他请求的工作量下界。
- 以上都不能换算成 scan 加速倍数，更不能换算成 TBT。真实时延还受 PV、命令时序和共享资源等待影响。

## 4. 为什么仍不能承诺 TBT，怎样读 sweep

假设仅作解释的串行模型 `T=G+S`，scan 降幅为 g，则 TBT 降幅为 `g×S/(G+S)`。scan 降 15% 而想得到 5% 的 TBT，scan 必须占原 TBT 的三分之一。当前 LLAMA3-8B、实际 batch=6 的 GPU 线性层/norm/activation 解析工作量约 187.888 µs/层；用全部 GPU 工作作量级参考，需要约 93.944 µs/层的 scan 才满足这个例子。head 流水会重叠一部分 GPU 工作，所以这个 G 不是实测关键路径，也不是 W1 的 TBT 预测。
...
**decode 的跨会话 MQ 当前没有按指南预期发生。** batch8 会把每轮六个请求放入一个组；代码要求全组的 master 交集非空才构造共同扫描。两对 worker 各自共读的文档，main 和另一对 worker 并不读。主审核到末轮每对 worker 共读 6144 token，但六人交集为 0。独立 agent 还核过全部 tier/output-step，整组交集始终为空。
...
- 末轮 main：16 次 sweep，平均每次实际 Ramulator 时间低于 18.883 µs，PIM 服务价才优于 GPU。
- 末轮 worker：12 次 sweep，门槛 26.602 µs。
- corpus 导入含前缀，共 12544 token，需要 6272 次 sweep，门槛仅 2.019 µs。它与后期低-query请求形成了合理的选边对照。
- 首轮 main 也是 fresh 小请求，其门槛只有 0.040 µs；不能写成“除了 corpus，其他每轮都一定选 PIM”。
...
## 8. 执行回复（chenyi9 指示：对的改、错的在此回复，2026-09-06）

| 第 6 节事项 | 判断 | 处理 |
|---|---|---|
| 连续 packed diff 的扫描段未合并 | 属实 | 已改：`PhysicalLedger.extent_groups` 在每个通道内按地址把 K/V 都连续的段合并成一个扫描描述符，对象身份仍在账本索引里，所有 policy 同一规则；新增测试 `test_contiguous_packed_repairs_form_one_scan_segment`（两个背靠背的 8-token 修正在 A3b/A4c/A4e 下都是一段 16 token） |
| W1 两两共读未被整批 MQ 利用 | 属实：整批交集为空 | 文档已改口（`workload/probe/README.md`、协议 §2.1）：W1 上 A5/A6 的 MQ 来自 prefill sweep 和 GQA；是否让实现按共读子组拆 sweep 是设计决定，未改代码。workload 侧的替代是让 batch 全体共读一份材料（例如所有人都读的简报），等 chenyi9 定 |
| A4e 评分不是当前读集的最少行 | 属实（措辞与实现不一致） | 按实现定义措辞：得分 = 该 agent 各轮请求读取该通道 master 块的累计次数 + 已分配给它的 diff 行数，并列沿用轮转；README、贡献、设计阶梯与论文 04 节已改。实现未动：按各轮累计计数是有意的（每轮都重扫整段上下文的块权重更大） |
| 尾列请求越过 extent | 属实，是 AttAcc 粒度近似 | 不改：沿用已裁决的共同近似（PARTIAL_COLUMN_APPEND_AUDIT） |
| 论文 06/07 节与图未同步 | 属实 | 已改：06 节 comparison points（A3b/A4c/A4e/A5/A6 措辞）、workloads（W1 与 12 个变体）、models（每 GPU 5 栈、各 model 的通道数、186 次运行）；07 节 e2e 与 ladder 引用；04 节把 j mod s 明确为默认轮转、表覆盖。图仍待重画（TODO 保留） |
| 结构探针被当性能证据 | 探针自身已标明校准来源 | 不改代码；文档只把它作结构线索 |
| main 读 r−2 轮、末两轮回答未被消费 | 属实 | README 补一句语义说明；W1 不改 |

未跑任何性能仿真；上述实现改动需重跑 W1 才有新数。

## 9. 对执行回复的审阅

chenyi9 要求核对修复及“不改”的理由。[本轮复核](EXECUTION_REPLY_REVIEW.md)确认连续段合并在实际W1地址上成立；A4e明示累计评分、MQ和延迟汇总的文案处理有依据。尾列取整可以作为保留近似，但当前仍有布局相关的额外行访问，不能把“同源公式”写成“各档误差相同”或“存储/扫描已严格一致”。新证据独立保存在 `execution_reply_review/`，上面的修复前计数与第8节执行回复按原记录保留。
