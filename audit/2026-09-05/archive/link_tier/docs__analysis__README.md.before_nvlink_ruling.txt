...
对 GQA=4，回读量大于返回量的条件为 `R>2m`。带宽仍高于远端 HBM 可供数据的速度时，继续降带宽也可能暂时不改变服务时间。

容量阈值建议按快层**净 KV 字节容量**推导，而不是为拉开结果挑 context 阈值。全模型原始 KV：`bytes/token = 2 × layers × KV_heads × head_dim × word_bytes`。仓库 LLAMA3-8B、fp16、TP=1 得到每 token 131,072 B（128 KiB）：

| 假设快层净 KV 容量 | 对应不含额外 padding/diff 的唯一 token 容量 |
|---|---:|
| 8 GiB | 65,536 |
| 16 GiB | 131,072 |
| 32 GiB | 262,144 |
