> 已归档的旧修改建议；不是当前整改决定。请看 [当前事项](../CURRENT_ISSUES.md)。

# Fugue 七档实验：修改建议与验收标准

**当前裁决优先：** chenyi9 要求先检查 AttAcc 本身是否建模、当前是否共同沿用、是否引入分档差异，再逐项决定是不是问题。共同限制不再列为整改，FlashAttention 必须开启。完整 [审阅清单](ATTACC_RELATIVE_FAIRNESS_REVIEW.md) 替代下文旧的 P0/P1 自动整改顺序；其“建议保留”仍待用户裁决，并非修复授权。理由与范围见 [session](../../../docs/sessions/2026-09-05-attacc-relative-fairness-ruling.md)。

**最新只读复核（`8c51672`）：** 见 [主报告](REAUDIT_8c51672.md)、[独立公平性报告](INDEPENDENT_REAUDIT_8c51672.md)、[计量专项](MODEL_PROVENANCE_REAUDIT_8c51672.md)。持久 scan row、零 diff master 几何、共同随机修正、STORE 零成本、A1 精确 L、A2 驻留回读、owner 依赖、MQ 接入和 A6 全 lane/尾批均已修。仍有 GQA validator/批量字节、ALL-BANK diff 区与子段转译、写入账本接入、A6 空回读和 Q variants 估价、共享库缓存及 tier 汇总问题。本次只审计，没有实施代码修复。

本文汇总整改建议；各项是否完成以最新报告的已修对照与 V01–V08 为准，不能将原建议全部视作当前未修。chenyi9 的新裁决是普通 DRAM store/read 不另收费、STORE 只保依赖，原先依赖普通 DRAM 额外带宽/能量的建议由此替代。A6 接受简单逐 request 选边，不要求两套候选 DAG。

**共同运行口径：** GPU 共用 FlashAttention，必须启用；当前默认仍是 legacy，README 示例已补参数，实现默认未改。pipeline 保持当前开启配置；Python/native 的共同追加顺序近似不再要求直接改成更强调度。配置缺记录先如实注明，只有证实混用影响比较时另交裁决。flash 的论文图近似模型有来源即可共同使用，不要求替换成实测。原 RC01–RC04 证据见 [运行补审](RUNTIME_CONFIGURATION_AUDIT.md)。

**已落地的计量修正：** 按 Fugue 正文删除 DIE query 旋转/position-transform，默认旋转归 GPU；TLB 是 A3b 起共同需要的寻址机制，各档统一不额外计 latency/energy。其余新增 DIE bookkeeping 也不另收取原始 AttAcc 没有的开销，只保留依赖且不占用调度资源。原始 AttAcc 的 PIM 命令、传输与 energy table 保留。详见 [PIM 计量来源核查](PIM_TIMING_PROVENANCE.md)。

这次修改的用户依据、技术原因、逐文件变化和验证边界，完整记录在 [session 文档](../../../docs/sessions/2026-09-05-attacc-accounting-and-rotation.md)。

审计对象为 `8750b5b`，原始反例、源码定位和统计见 [审计报告](REPORT.md)、[复现脚本](reproduce.py) 与 [证据](evidence.json)。本文件对“什么改动可以接受”的定义优先于原报告中的初始要求。
...
旧版 `main.py` 按 `kv_mapping != naive` 改变修正索引的问题已修：当前 CLI 固定 `recompute_canonical=False`，各复用档共用随机计划。以下保留逻辑计划独立于布局的要求。

建议：

1. 按 chenyi9 最新裁决，正式复用 ladder 共用随机 k 行的计划、相同 seed 与索引集合，不要求改回 canonical 前 k 行或换回另一种 policy。
2. 已删除“按档位改变逻辑修正位置”的 CLI 分支；保持布局只改变物理地址，不改变修正 token 身份。
...
**当前验收：** 旧反例的修正集合差异已消除。继续保持仅改变 rung 时，逻辑 KV、修正索引、请求长度、输出长度、到达/依赖一致。

### 3.2 A1/A2 可以独立设计，但必须执行自己声明的工作

A1 当前已执行论文中的 GPU prefill / PIM decode，decode 使用精确 L。`_append_physical_no_reuse_prefill_layer()` 只供显式 PIM-prefill 对照，不是 A1 preset 的默认路径；设备路由的旧反例已关闭。
...
当前已给所有 active lane 定价后取最慢者，并按真实 query 数给尾批估价。保持估价不迁移 KV、不提交实际事件或更改另一侧输入。

GPU 分支的无用途 Q-to-PIM 已移除。剩余估价缺口是零驻留回读仍被询价、PIM 实际 Q variants 的额外链路尚未进入估价；见最新 V06。两侧相同的公共成本可消去，不同操作应与执行分支对应。第一层估计跨层使用可以保留为已披露的简化；STORE/DIE/TLB 各档均不额外计价。
