# 实验指导：八通道、k4/k8 与四项延迟指标

机制解释见 [性能分析](../analysis/README.md)，实现核查见 [审计入口](../audit/README.md)。以下是执行说明，本次文档整理没有启动性能任务。

## 1. 固定条件

| 项目 | 当前口径 |
|---|---|
| 模型/硬件 | `LLAMA3-8B --ngpu 1 --num-hbm 4`：8 个 KV head，每 HBM 2 个，每 head 8 个 channel；TINY 可验证流程，结果单独标注 |
...
| 复用 | A2–A6 同一点统一 `--reuse recompute`、k4 或 k8；A1 用 no-reuse，不传 k |
...
| 要回答什么 | 已有输入/控制 | 比较档位 |
|---|---|---|
| 完整阶梯 | `B1_turns.json`，分别 k4/k8 | 七档 |
| diff 聚合 | B1、`T1_agents_4_turns.json`、`T3_chunks_1_turns.json` | A3b、A4c |
| 软件表 | B1 散取、`T6_retrieval_consecutive_turns.json` | A4c、A4e，可带 A3b |
| k4 与选边 | 同一个 B1 k4/k8；`T5_chatty_share_0p0_turns.json`、`T5_chatty_share_1p0_turns.json` 控制 worker 新计算量 | A4e、A5、A6 |
| 输出长度不均 | `T9_lout_mix_32_turns.json`、`T9_lout_mix_8_turns.json` | 归因布局需 A3b/A4c/A4e，归因选边需 A4e/A5/A6 |
...
for FUGUE_K in 4 8; do
    ...
    for FUGUE_RUNG in $FUGUE_RUNGS; do
        ...
        python3 main.py \
            ...
            --pim-link nvlink3 --num-hbm 4 --ngpu 1 \
...
这两个入口固定 `--workload-report-events none`，适合已有 E2E/TBT/能量汇总，单凭其产物不够生成完整 scan latency。参数可设成八通道/k4：

```bash
GPU_MODEL=flash EPIC_K=4 NUM_HBM=4 NGPU=1 RAMU_WORKERS=7 \
RUNGS="A4e A5 A6" \
KVPIM_PREFILL_SIDE_LOG="$KVPIM_SCRATCH/B1_8ch_k4.sides.jsonl" \
bash experiments/run_dag_ladder.sh workload/probe/sweep/B1_turns.json \
    LLAMA3-8B "$KVPIM_SCRATCH/B1_8ch_k4_summary"

GPU_MODEL=flash EPIC_K=4 NUM_HBM=4 NGPU=1 \
RUNGS="A4e A5 A6" PARALLEL=1 RAMU_WORKERS=7 \
bash experiments/run_sweep.sh "$KVPIM_SCRATCH/short_prefill_8ch_k4" \
    '^(B1_turns|T5_chatty_share_.*_turns)[.]json$' LLAMA3-8B
```

当前 sweep 仅把 B0_* 自动视为七档 baseline，B1 默认只跑 A3b/A6；要归因 A5/A6 必须显式 RUNGS。这些 summary 入口与上一节 full 入口的产物粒度不同，不能把 lane-sum 改名为 scan latency。
