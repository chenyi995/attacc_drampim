...
DEFAULT_STAGE = ROOT/'archive/coverage-expansion/attacc-fugue'
if 'fugue.kvchime_traces' not in sys.modules:
    sys.path.insert(0, str(DEFAULT_STAGE))
...
def micro_worker(spec_path):
    spec = json.loads(Path(spec_path).read_text())
    os.environ['LD_LIBRARY_PATH'] = spec['runtime']+os.pathsep+os.environ.get('LD_LIBRARY_PATH', '')
    from fugue.kvchime_model import geometry
    bank = ExactTraceBank(spec['root'], spec['runtime'], geometry(spec['model']), spec['cache_root'])
    row = bank.run(spec['name'], Path(spec['trace']).read_text().splitlines(),
                   spec['residents'], spec['metadata'])
    save(spec['result'], row)


def self_test(output):
    """A small real native trace, with independent uncached comparisons."""
    from fugue.kvchime_model import geometry
    output = Path(output).resolve(); output.mkdir(parents=True, exist_ok=False)
    runtime = output/'runtime'; runtime.mkdir()
    build_path = ROOT.parent/'attacc-fugue/output/KVChime-multi-model-TP4-20260907/build/Fugue-asplos-build.json'
    build = json.loads(build_path.read_text())
    for source, name, field in [('binary', 'ramulator2', 'binary_sha256'), ('library', 'libramulator.so', 'library_sha256')]:
        if digest(build[source]) != build[field]: raise CacheIntegrityError('Reference build changed')
        shutil.copy2(build[source], runtime/name)
    os.environ['LD_LIBRARY_PATH'] = str(runtime)+os.pathsep+os.environ.get('LD_LIBRARY_PATH', '')
    original = native.TraceBank(output/'uncached', runtime, geometry('LLAMA-7B'))
    lines = original.base(64, 1)
    residents = [1]*sum(line.startswith('PIM_MAC_AB') for line in lines)
    reference = original.run('physical', lines, residents)
    key_events = list(ExactTraceBank.iter_key_events(dict(canonical_path=reference['path'])))
    addresses = sorted(set(position*32 for position, _ in key_events))
    assert len(addresses) > 1
    metadata = []
    for label, address in [('first', addresses[0]), ('last', addresses[-1])]:
        metadata.append(dict(objects=[dict(id=label, private=True)],
                             physical_layout={label: dict(K_base=address, span_bytes=32)}))
    originals = [original.run('logical-'+str(i), lines, residents, item) for i, item in enumerate(metadata)]
    assert originals[0]['first_private_k_us'] != originals[1]['first_private_k_us']
    cache = output/'shared-cache'
    cached = ExactTraceBank(output/'logical-a', runtime, geometry('LLAMA-7B'), cache, base_cache_bytes=1)
    # Verify bounded eviction does not break inherited get/parallel prepare.
    assert cached.base(64, 1) == lines and cached.base(64, 1) == lines
    miss = cached.run('request-a', lines, residents, metadata[0])
    hit = cached.run('request-b', lines, residents, metadata[1])
    fields = ['cycles', 'scan_us', 'nCCDAB', 'mac_commands', 'represented_column_MACs',
              'dram_read_bytes_per_attacc', 'memory_energy_pJ_per_attacc',
              'query_movement_commands', 'softmax_commands', 'first_private_k_us']
    assert not miss['cache_hit'] and hit['cache_hit']
    for actual, expected in zip([miss, hit], originals):
        assert {key: actual[key] for key in fields} == {key: expected[key] for key in fields}
    assert list(cached.iter_key_events(hit)) == reference['key_events']
    assert not cached.bases and all('key_events' not in row for row in cached.native_commands)
    trace = output/'test.trace'; trace.write_text('\n'.join(lines)+'\n')
    def spec(label, cache_root, model):
        path = output/(label+'.json')
        save(path, dict(root=str(output/label), runtime=str(runtime), model=model,
             cache_root=str(cache_root), name=label, trace=str(trace), residents=residents,
             metadata=metadata[1], result=str(output/(label+'-result.json'))))
        return path
    child = spec('new-process-model', cache, 'GPT-13B')
    subprocess.run([sys.executable, __file__, '--micro-worker', str(child)], check=True)
    cross = json.loads((output/'new-process-model-result.json').read_text())
    assert cross['cache_hit'] and cross['cache_key'] == miss['cache_key']
    assert {key: cross[key] for key in fields} == {key: originals[1][key] for key in fields}
    concurrent_cache = output/'concurrent-cache'
    processes = [subprocess.Popen([sys.executable, __file__, '--micro-worker', str(spec(label, concurrent_cache, model))])
                 for label, model in [('worker-one', 'LLAMA-7B'), ('worker-two', 'GPT-13B')]]
    assert all(process.wait() == 0 for process in processes)
    concurrent = [json.loads((output/(label+'-result.json')).read_text()) for label in ['worker-one', 'worker-two']]
    assert sorted(row['cache_hit'] for row in concurrent) == [False, True]
    assert all(row['cache_key'] == miss['cache_key'] for row in concurrent)
    canonical = Path(miss['canonical_path'])
    corrupted_cache = output/'corrupt-cache'; shutil.copytree(cache, corrupted_cache)
    with (corrupted_cache/'profiles'/canonical.name/'stdout.log').open('a') as stream: stream.write('\ncorruption test\n')
    try:
        ExactTraceBank(output/'reject', runtime, geometry('LLAMA-7B'), corrupted_cache).run('reject', lines, residents, metadata[0])
    except CacheIntegrityError:
        corruption_rejected = True
    else:
        raise AssertionError('Corrupt completed cache was accepted')
    report = dict(all_checks_pass=True, scope='Small native command trace only; no request experiment.',
                  script_sha256=digest(__file__), runtime_binary_sha256=digest(runtime/'ramulator2'),
                  runtime_library_sha256=digest(runtime/'libramulator.so'),
                  exact_uncached_numeric_comparisons=fields, cache_key=miss['cache_key'],
                  first_private_k_us=[row['first_private_k_us'] for row in originals],
                  metadata_specific_private_time=True, cross_model_process_hit=True,
                  concurrent_single_native_run=True, corruption_rejected=corruption_rejected,
                  bounded_base_cache=True, key_events_stream_equal=True,
                  physical_profile=str(canonical), output=str(output))
    save(output/'checks.json', report)
...
if __name__ == '__main__':
    ...
    group.add_argument('--self-test', type=Path, help='New isolated output directory for a small real trace')
    group.add_argument('--micro-worker', type=Path, help=argparse.SUPPRESS)
    arguments = parser.parse_args()
    if arguments.self_test: self_test(arguments.self_test)
    else: micro_worker(arguments.micro_worker)
